...
function childPlainText(node, separator, keepEmpty = false) {
  ...
  const texts = node.children.map(
    (child) => plainTextNode(child)
  );
  return (keepEmpty ? texts : texts.filter((text2) => text2.length > 0)).join(
    separator
  );
}
