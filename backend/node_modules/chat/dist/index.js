...
var ThreadHistoryCache = class {
  ...
  async append(threadId, message) {
    ...
    for (let current = serialized; current; current = current.replyTo) {
      current.raw = null;
    }
...
var Chat = class {
   ...
   */
  async debounceLoop(lock, adapter, lockKey) {
   ...
   */
  async drainQueue(lock, adapter, lockKey) {
    while (true) {
      const pending = [];
      while (true) {
        const entry = await this._stateAdapter.dequeue(lockKey);
      ...
      const messageThreadId = latest.message.threadId;
      const skipped = pending.slice(0, -1).map((entry) => entry.message).filter((message) => message.threadId === messageThreadId);
      this.logger.info("message-dequeued", {
        threadId: messageThreadId,
        lockKey,
        messageId: latest.message.id,
