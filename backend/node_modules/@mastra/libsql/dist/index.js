...
var LibSQLVector = class extends MastraVector {
	...
	*/
	...
	async query({ indexName, queryVector, topK = 10, filter, includeVector = false, minScore = -1 }) {
		...
		if (!Array.isArray(queryVector) || !queryVector.every((x) => typeof x === "number" && Number.isFinite(x))) throw new MastraError({
	...
	*/
	...
	async doUpdateVector(params) {
		...
		if ("id" in params && params.id) {
			...
			if (!filterSql || filterSql.trim() === "") throw new MastraError({
			...
			if ([
				...
				"1 = 1",
				"1=1"
			].includes(normalizedCondition)) throw new MastraError({
...
function prepareWhereClause(filters, schema) {
	...
	for (const [columnName, filterValue] of Object.entries(filters)) {
		...
		const result = buildCondition(parseSqlIdentifier(columnName, "column name"), filterValue);
...
function transformFromSqlRow({ tableName, sqlRow }) {
	const result = {};
	const jsonColumns = new Set(Object.keys(TABLE_SCHEMAS[tableName]).filter((key) => TABLE_SCHEMAS[tableName][key].type === "jsonb").map((key) => key));
	const dateColumns = new Set(Object.keys(TABLE_SCHEMAS[tableName]).filter((key) => TABLE_SCHEMAS[tableName][key].type === "timestamp").map((key) => key));
	const booleanColumns = new Set(Object.keys(TABLE_SCHEMAS[tableName]).filter((key) => TABLE_SCHEMAS[tableName][key].type === "boolean").map((key) => key));
...
var LibSQLDB = class extends MastraBase {
	...
	*/
	async select({ tableName, keys }) {
		...
		const conditions = Object.keys(keys).map((key) => parseSqlIdentifier(key, "column name")).map((key) => `${key} = ?`).join(" AND ");
	...
	*/
	async selectTotalCount({ tableName, whereClause }) {
		const statement = `SELECT COUNT(*) as count FROM ${parseSqlIdentifier(tableName, "table name")} ${whereClause ? `${whereClause.sql}` : ""}`;
		const result = await this.client.execute({
			sql: statement,
			args: whereClause?.args ?? []
		});
	...
	*/
	async createTable({ tableName, schema, compositePrimaryKey }) {
		try {
			...
			if (tableName === TABLE_SPANS) tableConstraints.push("UNIQUE (spanId, traceId)");
			...
			if (tableName === TABLE_SPANS) await this.migrateSpansTable();
		} catch (error) {
	...
	async migrateSpansTable() {
		...
		try {
			...
			for (const [columnName, columnDef] of Object.entries(schema)) if (!existingColumns.has(columnName.toLowerCase())) {
				const alterSql = `ALTER TABLE "${TABLE_SPANS}" ADD COLUMN "${columnName}" ${this.getSqlType(columnDef.type)}`;
				await this.client.execute(alterSql);
				addedColumns = true;
				this.logger.debug(`LibSQLDB: Added column '${columnName}' to ${TABLE_SPANS}`);
			}
			if (addedColumns) this.tableColumnsCache.delete(TABLE_SPANS);
...
MIGRATION REQUIRED: Duplicate spans detected in ${TABLE_SPANS}\n===========================================================================\n\nFound ${duplicateInfo.duplicateCount} duplicate (traceId, spanId) combinations.\n\nThe spans table requires a unique constraint on (traceId, spanId), but your\ndatabase contains duplicate entries that must be resolved first.\n\nTo fix this, run the manual migration command:\n\n  npx mastra migrate\n\nThis command will:\n  1. Remove duplicate spans (keeping the most complete/recent version)\n  2. Add the required unique constraint\n\nNote: This migration may take some time for large tables.\n===========================================================================\n`;
					...
					await this.client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS "mastra_ai_spans_spanid_traceid_idx" ON "${TABLE_SPANS}" ("spanId", "traceId")`);
	...
	*/
	async pruneBatch({ tableName, column, cutoff, limit }) {
		...
		const sql = `DELETE FROM "${parsedTable}" WHERE rowid IN (SELECT rowid FROM "${parsedTable}" WHERE "${parseSqlIdentifier(column, "column name")}" < ? LIMIT ?)`;
	...
	*/
	async pruneUnitsBatch({ parentTable, parentKey, parentColumn, childTable, childForeignKey, cutoff, limit }) {
		...
		const agedParents = `SELECT "${parsedParentKey}" FROM "${parsedParent}" WHERE "${parseSqlIdentifier(parentColumn, "column name")}" < ? ORDER BY rowid LIMIT ?`;
...
var AgentsLibSQL = class extends AgentsStorage {
	...
	*/
	async #migrateFromLegacySchema() {
		...
		if (await this.#db.hasColumn(TABLE_AGENTS, "name")) {
			await this.#client.execute({ sql: `ALTER TABLE "${TABLE_AGENTS}" RENAME TO "${legacyTable}"` });
			await this.#client.execute({ sql: `DROP TABLE IF EXISTS "${TABLE_AGENT_VERSIONS}"` });
		}
	...
	*/
	async #migrateVersionsSchema() {
		if (!await this.#db.hasColumn(TABLE_AGENT_VERSIONS, "snapshot")) return;
		await this.#client.execute({ sql: `DROP TABLE IF EXISTS "${TABLE_AGENT_VERSIONS}"` });
		await this.#client.execute({ sql: `DROP TABLE IF EXISTS "${TABLE_AGENTS}_legacy"` });
	}
	/**
	...
	*/
	async #cleanupStaleDrafts() {
		try {
			await this.#client.execute({ sql: `DELETE FROM "${TABLE_AGENTS}"
              WHERE status = 'draft'
	...
	*/
	async #migrateToolsToJsonbFormat() {
		try {
			const result = await this.#client.execute({ sql: `SELECT id, tools FROM "${TABLE_AGENT_VERSIONS}" WHERE tools IS NOT NULL` });
	...
	async create(input) {
		...
		try {
			...
			if (!created) throw new MastraError({
	...
	async list(args) {
		...
		try {
			...
			if (metadata && Object.keys(metadata).length > 0) for (const [key, value] of Object.entries(metadata)) {
				if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(key)) throw new MastraError({
...
var BackgroundTasksLibSQL = class BackgroundTasksLibSQL extends BackgroundTasksStorage {
	/**
...
var DatasetsLibSQL = class extends DatasetsStorage {
	...
	async init() {
		...
		await this.#db.createTable({
			...
			schema: DATASET_VERSIONS_SCHEMA
		});
	...
	async createDataset(input) {
		try {
			...
			if (input.id !== void 0 && hasErrorCode(error, /* @__PURE__ */ new Set([
				"SQLITE_CONSTRAINT",
	...
	async deleteDataset({ id, filters }) {
		try {
			...
			statements.push({
				sql: `DELETE FROM ${TABLE_DATASETS} WHERE ${scopedWhere}`,
				args: [id, ...params]
			});
	...
	async listDatasets(args) {
		try {
			...
			if (args.filters?.targetIds !== void 0 && args.filters.targetIds.length > 0) {
				...
				filterConditions.push(`EXISTS (SELECT 1 FROM json_each(${TABLE_DATASETS}.targetIds) WHERE value IN (${placeholders}))`);
	...
	*/
	...
	async deleteExperiment(args) {
		try {
			...
			await this.#client.batch([{
				sql: `DELETE FROM ${TABLE_EXPERIMENT_RESULTS} WHERE ${cascadeWhere}`,
				args: cascadeArgs
			}, {
				sql: `DELETE FROM ${TABLE_EXPERIMENTS} WHERE ${parentScoped.sql}`,
...
var FavoritesLibSQL = class extends FavoritesStorage {
	...
	async init() {
		...
		await this.#client.execute(`CREATE INDEX IF NOT EXISTS idx_favorites_entity ON "${TABLE_FAVORITES}" ("entityType", "entityId")`);
	}
	async dangerouslyClearAll() {
		const tx = await this.#client.transaction("write");
		try {
...
var HarnessLibSQL = class HarnessLibSQL extends HarnessStorage {
...
var MCPClientsLibSQL = class extends MCPClientsStorage {
	...
	async init() {
		await this.#db.createTable({
			...
			schema: MCP_CLIENTS_SCHEMA
		});
		await this.#db.createTable({
			tableName: TABLE_MCP_CLIENT_VERSIONS,
			schema: MCP_CLIENT_VERSIONS_SCHEMA
		});
		await this.#client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_mcp_client_versions_client_version ON "${TABLE_MCP_CLIENT_VERSIONS}" ("mcpClientId", "versionNumber")`);
	}
	async dangerouslyClearAll() {
...
var MCPServersLibSQL = class extends MCPServersStorage {
	...
	async init() {
		...
		await this.#client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_mcp_server_versions_server_version ON "${TABLE_MCP_SERVER_VERSIONS}" ("mcpServerId", "versionNumber")`);
	}
	async dangerouslyClearAll() {
...
var MemoryLibSQL = class MemoryLibSQL extends MemoryStorage {
	...
	*/
	...
	async listMessagesById({ messageIds }) {
      ...
      `;
			...
			return { messages: new MessageList().add(result.rows.map(this.parseRow), "memory").get.all.db() };
		} catch (error) {
			throw new MastraError({
	...
	async updateMessages({ messages }) {
		...
		const selectSql = `SELECT * FROM ${TABLE_MESSAGES} WHERE id IN (${messageIds.map(() => "?").join(",")})`;
	...
	async listThreads(args) {
		...
		try {
			...
			const baseQuery = `FROM ${TABLE_THREADS} ${whereClauses.length > 0 ? `WHERE ${whereClauses.join(" AND ")}` : ""}`;
		...
		try {
			await withClientWriteLock(this.#client, async () => {
				...
				try {
					...
					if (selectResult.rows.length === 0) throw new MastraError({
		...
		try {
			...
			await withClientWriteLock(this.#client, async () => {
				...
				try {
					...
					if (!current.rows || current.rows.length === 0) throw new MastraError({
...
var NotificationsLibSQL = class NotificationsLibSQL extends NotificationsStorage {
...
var ObservabilityLibSQL = class ObservabilityLibSQL extends ObservabilityStorage {
	/**
	...
	async listTraces(args) {
		...
		try {
			...
			return {
				...
				spans: toTraceSpans(spans.map((span) => transformFromSqlRow({
					tableName: TABLE_SPANS,
					sqlRow: span
				})))
			};
...
var PromptBlocksLibSQL = class extends PromptBlocksStorage {
	...
	async init() {
		...
		await this.#client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_prompt_block_versions_block_version ON "${TABLE_PROMPT_BLOCK_VERSIONS}" ("blockId", "versionNumber")`);
	}
	async dangerouslyClearAll() {
...
var SchedulesLibSQL = class SchedulesLibSQL extends SchedulesStorage {
	/**
...
var ScorerDefinitionsLibSQL = class extends ScorerDefinitionsStorage {
	...
	async init() {
		await this.#db.createTable({
			...
			schema: SCORER_DEFINITIONS_SCHEMA
		});
		await this.#db.createTable({
			tableName: TABLE_SCORER_DEFINITION_VERSIONS,
			schema: SCORER_DEFINITION_VERSIONS_SCHEMA
		});
		await this.#client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_scorer_definition_versions_scorer_version ON "${TABLE_SCORER_DEFINITION_VERSIONS}" ("scorerDefinitionId", "versionNumber")`);
	}
	async dangerouslyClearAll() {
...
var ScoresLibSQL = class ScoresLibSQL extends ScoresStorage {
	/**
...
var SkillsLibSQL = class extends SkillsStorage {
	...
	async init() {
		...
		await this.#client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_skill_versions_skill_version ON "${TABLE_SKILL_VERSIONS}" ("skillId", "versionNumber")`);
	}
	async dangerouslyClearAll() {
...
*/
var ThreadStateLibSQL = class ThreadStateLibSQL extends ThreadStateStorage {
	/**
...
var ToolProviderConnectionsLibSQL = class extends ToolProviderConnectionsStorage {
	...
	async init() {
		...
		await this.#client.execute(`CREATE INDEX IF NOT EXISTS idx_tool_provider_connections_author ON "${TABLE_TOOL_PROVIDER_CONNECTIONS}" ("authorId", "providerId", "toolkit")`);
	}
	async dangerouslyClearAll() {
	...
	*/
	...
	async listWorkflowRuns({ workflowName, fromDate, toDate, page, perPage, resourceId, status } = {}) {
		try {
			...
			if (resourceId) if (await this.#db.hasColumn(TABLE_WORKFLOW_SNAPSHOT, "resourceId")) {
				conditions.push("resourceId = ?");
...
var WorkspacesLibSQL = class extends WorkspacesStorage {
	...
	async init() {
		...
		await this.#client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_workspace_versions_workspace_version ON "${TABLE_WORKSPACE_VERSIONS}" ("workspaceId", "versionNumber")`);
	}
	async dangerouslyClearAll() {
...
var LibSQLFactoryStorageOps = class {
	...
	async #insertOne(collection, row) {
		...
		const values = { ...row };
		if (schema.columns[pk].type === "uuid-pk" && values[pk] === void 0) values[pk] = randomUUID();
...
*/
var LibSQLFactoryStorage = class extends FactoryStorage {
	ops;
	#client;
	#config;
