import { i as isLeaseProvider, r as PubSub } from "./event-emitter-KtHd9pSx.js";
	...
	*/
	async subscribeFromOffset(topic, offset, cb) {
		...
		const wrappedCb = (event, ack, nack) => {
			if (typeof event.index === "number" && event.index < offset) return ack?.();
			if (bootstrapping) {
				buffer.push({
					event,
					ack,
			...
			if (typeof event.index === "number" && event.index <= lastDelivered && !isRetry) return ack?.();
...
*/
const RUN_LOCAL_TOPIC_PREFIXES = ["workflow.events.v2."];
/**
...
*/
function isRunLocalTopic(topic) {
	return RUN_LOCAL_TOPIC_PREFIXES.some((prefix) => topic.startsWith(prefix));
}
//#endregion
export { withCaching as i, isRunLocalTopic as n, CachingPubSub as r, RUN_LOCAL_TOPIC_PREFIXES as t };

//# sourceMappingURL=topics-BCcUoD5n.js.map