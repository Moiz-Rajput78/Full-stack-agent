...
async function executeAgentSchedule(mastra, scheduleId, target, ctx = {}) {
	...
	const resolved = await require_resolve_agent.resolveAgentById(mastra, agentId);
	if (resolved.status === "error") {
		log?.error?.("Failed to resolve stored agent for schedule", {
			scheduleId,
			agentId,
	...
	const prepareStages = [];
	if (fsHandler) prepareStages.push(() => Promise.resolve(fsHandler({
		mastra,
		agentId,
		scheduleId,
		key: require_define.parseFsAgentScheduleRowId(scheduleId)?.key ?? scheduleId,
		trigger
	})));
	if (hooks?.prepare) prepareStages.push(() => Promise.resolve(hooks.prepare({
		mastra,
		agentId,
		schedule: scheduleRef,
		trigger
	})));
	let effective = buildEffectiveFromTarget(target);
	for (const stage of prepareStages) {
		...
		effective = mergeEffective(effective, result ?? void 0);
	}
...
//# sourceMappingURL=worker-DDjMxBCK.cjs.map