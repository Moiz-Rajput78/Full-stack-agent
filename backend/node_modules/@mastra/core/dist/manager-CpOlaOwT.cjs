...
const require_trip_wire = require("./trip-wire-BG9Psy6j.cjs");
const require_agent = require("./agent-NkjbehmN.cjs");
const require_constants = require("./constants-CWB45krq.cjs");
...
async function executeExperimentItem(args) {
	...
	if (experiment.targetType === null || experiment.targetId === null) throw new require_error.MastraError({
	...
	if (!resolved) throw new require_error.MastraError({
	...
	const storage = mastra.getStorage();
	const experimentsStore = await storage?.getStore("experiments");
	if (!experimentsStore) throw new require_error.MastraError({
...
var Dataset = class {
	...
	*/
	async createExperiment(args) {
		...
		if (hasTargetType !== (args?.targetId !== void 0)) throw new require_error.MastraError({
		...
		if (args?.scorers?.length && !hasTargetType) throw new require_error.MastraError({
			id: "EXPERIMENT_INVALID_TARGET",
		...
		if (hasTargetType) {
			if (!await resolveTarget(this.#mastra, args.targetType, args.targetId)) throw new require_error.MastraError({
				id: "EXPERIMENT_TARGET_NOT_FOUND",
		...
		if (args?.id) {
			...
			if (existing) {
				if (!(existing.datasetId === this.id && existing.targetType === (args.targetType ?? null) && existing.targetId === (args.targetId ?? null))) throw new require_error.MastraError({
	...
	*/
	async runExperimentItem(args) {
		...
		if (experiment.targetType === null) throw new require_error.MastraError({
			id: "EXPERIMENT_HAS_NO_TARGET",
			text: `Experiment ${args.experimentId} has no target; ingest results via submitExperimentResult instead`,
			domain: "STORAGE",
			category: "USER"
		});
		if (experiment.status === "completed" || experiment.status === "failed") throw new require_error.MastraError({
		...
		if (!item || item.datasetId !== this.id) throw new require_error.MastraError({
...
Object.defineProperty(exports, "executeExperimentItem", {
	enumerable: true,
	get: function() {
		return executeExperimentItem;
	}
...
Object.defineProperty(exports, "resolveTarget", {
	enumerable: true,
	get: function() {
...
//# sourceMappingURL=manager-CpOlaOwT.cjs.map