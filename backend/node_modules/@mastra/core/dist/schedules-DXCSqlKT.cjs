...
const require_slugify = require("./slugify-nMKZf7GM.cjs");
...
function canonicalizeScheduleId(rawId, prefix) {
	...
	const slug = require_slugify.slugify(trimmed.startsWith(prefix) ? trimmed.slice(prefix.length) : trimmed);
	if (!slug) return "";
	return `${prefix}${slug}`;
}
/**
...
function normalizeScheduleId(rawId, prefix) {
	...
	if (!canonical) throw new require_error.MastraError({
...
var Schedules = class {
	...
	*/
	async #load(id) {
		...
		const canonical = canonicalizeScheduleId(trimmed, require_define.AGENT_SCHEDULE_PREFIX);
	...
	async #createAgentSchedule(input) {
		require_cron.validateCron(input.cron, input.timezone);
		if (!input.agentId) throw new require_error.MastraError({
		...
		if (input.threadId && !input.resourceId) throw new require_error.MastraError({
		...
		if (!input.threadId) {
			...
			if (offenders.length > 0) throw new require_error.MastraError({
		...
		const id = input.id !== void 0 ? normalizeScheduleId(input.id, require_define.AGENT_SCHEDULE_PREFIX) : `${require_define.AGENT_SCHEDULE_PREFIX}${(0, crypto.randomUUID)()}`;
		await this.#assertIdAvailable(store, id, input.id !== void 0);
		const now = Date.now();
		const nextFireAt = require_cron.computeNextFireAt(input.cron, {
	...
	async #createWorkflowSchedule(input) {
		...
		const store = await this.#getStore();
		await this.#mastra.__ensureScheduleRuntimeReady();
		const id = input.id !== void 0 ? normalizeScheduleId(input.id, require_define.WORKFLOW_SCHEDULE_PREFIX) : `${require_define.WORKFLOW_SCHEDULE_PREFIX}${(0, crypto.randomUUID)()}`;
		await this.#assertIdAvailable(store, id, input.id !== void 0);
		const now = Date.now();
		const nextFireAt = require_cron.computeNextFireAt(input.cron, {
			timezone: input.timezone,
	...
	async #assertIdAvailable(store, id, callerProvided) {
		...
		if (await store.getSchedule(id)) throw new require_error.MastraError({
	...
	async update(id, patch) {
		...
		if (!existing) throw new require_error.MastraError({
		...
		if (patch.cron !== void 0 || patch.timezone !== void 0) require_cron.validateCron(nextCron, nextTimezone);
		...
		const nextFireAt = patch.cron !== void 0 || patch.timezone !== void 0 || resuming ? require_cron.computeNextFireAt(nextCron, {
	...
	async resume(id) {
		...
		const nextFireAt = require_cron.computeNextFireAt(existing.cron, {
...
Object.defineProperty(exports, "toAgentSchedule", {
	enumerable: true,
	get: function() {
		return toAgentSchedule;
	}
});
Object.defineProperty(exports, "toScheduleView", {
	enumerable: true,
	get: function() {
		return toScheduleView;
	}
});
Object.defineProperty(exports, "toWorkflowSchedule", {
	enumerable: true,
	get: function() {
...
//# sourceMappingURL=schedules-DXCSqlKT.cjs.map