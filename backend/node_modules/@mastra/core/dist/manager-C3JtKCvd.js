...
import { b as isSupportedLanguageModel } from "./trip-wire-DjMA49sx.js";
import { ln as ScorerRunError, sn as validateAndSaveScore } from "./agent-DSxJoGjY.js";
import { An as extractTrajectoryFromTrace, kn as extractTrajectory } from "./constants-CFR60Gmg.js";
...
//#endregion
//#region src/datasets/experiment/resolve-target.ts
/**
...
*/
function experimentScoreId(experimentId, itemId, attempt, scorerId) {
...
function experimentScoreKey(experimentId, itemId, attempt) {
	return `expscore:${experimentId}:${itemId}:${attempt}`;
}
async function runScorersForItem(scorers, item, output, storage, runId, targetType, targetId, itemId, scorerInput, scorerOutput, traceId, workflowData, persistScores = true, stableScoreKey) {
...
//#endregion
//#region src/datasets/experiment/item.ts
/**
...
*/
async function executeExperimentItem(args) {
	const { mastra, experiment, item } = args;
	const attempt = args.attempt ?? 0;
	if (experiment.targetType === null || experiment.targetId === null) throw new MastraError({
	...
	const resolved = await resolveTarget(mastra, experiment.targetType, experiment.targetId, experiment.agentVersion ?? void 0);
	if (!resolved) throw new MastraError({
	...
	if (experiment.scorerIds != null) scorers = resolveScorers(mastra, [...new Set(experiment.scorerIds)]);
	...
	} else scorers = resolveScorers(mastra, [...new Set(args.datasetScorerIds ?? [])]);
	const storage = mastra.getStorage();
	const experimentsStore = await storage?.getStore("experiments");
	if (!experimentsStore) throw new MastraError({
	...
	const mergedRequestContext = args.requestContext || item.requestContext ? {
		...args.requestContext,
		...item.requestContext
	} : void 0;
	const execResult = scorerConfigError ? {
		output: null,
	...
	if (!scorerConfigError) {
		...
		scores = await runScorersForItem(scorers, item, execResult.output, storage ?? null, experiment.id, experiment.targetType, experiment.targetId, item.id, execResult.scorerInput, execResult.scorerOutput, execResult.traceId ?? void 0, workflowData, true, experimentScoreKey(experiment.id, item.id, attempt));
	}
	return {
		result: await experimentsStore.upsertExperimentResult({
			experimentId: experiment.id,
			itemId: item.id,
			attempt,
...
var Dataset = class {
	...
	*/
	async #getOwnedExperiment(experimentId) {
		await this.#assertScope();
		const experiment = await (await this.#getExperimentsStore()).getExperimentById({
			id: experimentId,
			filters: this.#scope
	...
	*/
	async createExperiment(args) {
		...
		const datasetsStore = await this.#getDatasetsStore();
		const hasTargetType = args?.targetType !== void 0;
		if (hasTargetType !== (args?.targetId !== void 0)) throw new MastraError({
		...
		if (args?.scorers?.length && !hasTargetType) throw new MastraError({
			id: "EXPERIMENT_INVALID_TARGET",
		...
		if (hasTargetType) {
			if (!await resolveTarget(this.#mastra, args.targetType, args.targetId)) throw new MastraError({
				id: "EXPERIMENT_TARGET_NOT_FOUND",
		...
		if (args?.id) {
			const existing = await experimentsStore.getExperimentById({
				id: args.id,
				filters: this.#scope
			});
			if (existing) {
				if (!(existing.datasetId === this.id && existing.targetType === (args.targetType ?? null) && existing.targetId === (args.targetId ?? null))) throw new MastraError({
	...
	*/
	async runExperimentItem(args) {
		...
		if (experiment.targetType === null) throw new MastraError({
			id: "EXPERIMENT_HAS_NO_TARGET",
			text: `Experiment ${args.experimentId} has no target; ingest results via submitExperimentResult instead`,
			domain: "STORAGE",
			category: "USER"
		});
		if (experiment.status === "completed" || experiment.status === "failed") throw new MastraError({
		...
		const item = await datasetsStore.getItemById({
			id: args.itemId,
			datasetVersion: experiment.datasetVersion ?? void 0
		});
		if (!item || item.datasetId !== this.id) throw new MastraError({
	...
	*/
	async submitExperimentResult(args) {
		...
		const result = await experimentsStore.upsertExperimentResult({
			...
			input: args.input !== void 0 ? args.input : item.input,
			output: args.output ?? null,
			groundTruth: args.groundTruth !== void 0 ? args.groundTruth : item.groundTruth ?? null,
		...
		if (args.scores?.length) {
			const storage = this.#mastra.getStorage();
			if (storage) for (const score of args.scores) try {
				await validateAndSaveScore(storage, {
	...
	*/
	async finalizeExperiment(args) {
		const experiment = await this.#getOwnedExperiment(args.experimentId);
		if (experiment.status === "completed" || experiment.status === "failed") return experiment;
		const experimentsStore = await this.#getExperimentsStore();
		const itemSucceeded = /* @__PURE__ */ new Map();
		let page = 0;
		const perPage = 100;
		while (true) {
		...
		let succeededCount = 0;
		let failedCount = 0;
		for (const succeeded of itemSucceeded.values()) if (succeeded) succeededCount++;
		else failedCount++;
...
//#endregion
export { TOOL_MOCK_NOT_DECLARED as _, executeExperimentItem as a, computeScorerStats as c, resolveScorers as d, runScorersForItem as f, TOOL_MOCK_MISMATCH as g, TOOL_MOCK_EXHAUSTED as h, runExperiment as i, isRegression as l, executeTarget as m, Dataset as n, compareExperiments as o, resolveTarget as p, EXPERIMENT_ITEM_BEFORE_EACH_FAILED as r, computeMean as s, DatasetsManager as t, EXPERIMENT_ITEM_SCORER_NOT_FOUND as u, ToolMockMatcher as v };

//# sourceMappingURL=manager-C3JtKCvd.js.map