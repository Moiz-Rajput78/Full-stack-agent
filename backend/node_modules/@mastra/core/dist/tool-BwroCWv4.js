...
import { a as isZodObject, i as isZodArray, r as getZodTypeName, s as unwrapZodType } from "./zod-utils-DTkc-hhd.js";
//#region src/request-context/input-source.ts
/** @internal */
const REQUEST_CONTEXT_INPUT_SOURCE = Symbol.for("mastra.core.request-context.input-source");
/**
...
* @internal
*/
function getRequestContextInputSource(requestContext) {
...
/** @internal */
function getRequestContextInputValues(requestContext) {
	return getRequestContextInputSource(requestContext)?.all ?? {};
}
//#endregion
//#region src/tools/builder-validation-context.ts
const builderValidatedContexts = /* @__PURE__ */ new WeakSet();
function markBuilderValidatedInput(context) {
	builderValidatedContexts.add(context);
}
function consumeBuilderValidatedInput(context) {
	if (typeof context !== "object" || context === null || !builderValidatedContexts.has(context)) return false;
...
function coerceStringifiedJsonValues(schema, input) {
	...
	if (!isZodObject(unwrapped)) return input;
	...
	for (const [key, value] of Object.entries(input)) {
		...
		if (getZodTypeName(baseFieldSchema) === "ZodString") continue;
		const trimmed = value.trim();
		if (isZodArray(baseFieldSchema) && trimmed.startsWith("[") || isZodObject(baseFieldSchema) && trimmed.startsWith("{")) try {
...
function validateRequestContext(schema, requestContext, identifier) {
	if (!schema) return { data: getRequestContextInputValues(requestContext) };
	const contextValues = getRequestContextInputValues(requestContext);
	const validation = toStandardSchema(schema)["~standard"].validate(contextValues);
...
function getRequestContextInputValidator(schema) {
...
function getRequestContextEncoder(schema) {
	if (!schema || typeof schema !== "object" && typeof schema !== "function") return;
	const encodableSchema = schema;
	if (encodableSchema["~standard"]?.vendor !== "zod" || typeof encodableSchema.safeEncode !== "function") return;
	return (values) => {
		try {
			const result = encodableSchema.safeEncode(values);
			if (result.success && result.data && typeof result.data === "object" && !Array.isArray(result.data)) return result.data;
		} catch {}
	};
}
/**
...
*/
var TransformedRequestContext = class extends RequestContext {
	#source;
	#acceptsInput;
	#encode;
	constructor(source, transformedValues, acceptsInput, encode) {
	...
	#getSourceValue(key, value) {
		...
		if (this.#acceptsInput(nextSourceValues)) return value;
		const encodedValues = this.#encode?.({
			...this.all,
			[key]: value
		});
		if (encodedValues && Object.prototype.hasOwnProperty.call(encodedValues, key)) return encodedValues[key];
		throw new Error(`Unable to persist request context key "${key}": the value is not valid schema input and cannot be encoded from the schema output.`);
	}
	set(key, value) {
	...
	constructor(opts) {
		...
		if (opts.execute) {
			...
			this.execute = async (inputData, context) => {
				...
				const { data: validatedRequestContext, error: requestContextError } = validateRequestContext(this.requestContextSchema, sourceRequestContext, this.id);
				if (requestContextError) return requestContextError;
				const executionRequestContext = this.requestContextSchema ? new TransformedRequestContext(sourceRequestContext ?? new RequestContext(), validatedRequestContext, getRequestContextInputValidator(this.requestContextSchema), getRequestContextEncoder(this.requestContextSchema)) : context?.requestContext;
...
//#endregion
export { validateToolInput as a, markBuilderValidatedInput as c, isValidationError as i, getRequestContextInputValues as l, Tool as n, validateToolOutput as o, createTool as r, validateToolSuspendData as s, MASTRA_TOOL_MARKER as t };

//# sourceMappingURL=tool-BwroCWv4.js.map