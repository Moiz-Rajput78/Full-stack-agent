...
* lane.
*/
var ThreadStateStorage = class extends require_base.MastraBase {
	/**
...
function isClosable(value) {
	return typeof value === "object" && value !== null && typeof value.close === "function";
}
var MastraCompositeStore = class extends require_base.MastraBase {
	hasInitialized = null;
	...
	*/
	...
	constructor(config) {
		...
		if (config.default || config.editor || config.domains) {
			...
			for (const key of DOMAIN_KEYS) assign(key);
			if (domainOverrides.threadState !== false) composed.threadState = composed.threadState ?? new InMemoryThreadStateStorage();
			this.stores = composed;
		}
	}
	/**
	...
	*/
	...
	async #runInit() {
		...
		if (this.stores) {
			for (const domain of Object.values(this.stores)) if (typeof domain?.init === "function") maybeInit(domain);
		}
	...
	*/
	async close() {
		...
		const parentOwned = /* @__PURE__ */ new Set();
		const addParentDomains = (parent) => {
			if (!parent?.stores) return;
			for (const domain of Object.values(parent.stores)) if (domain) parentOwned.add(domain);
		};
		addParentDomains(this.parentDefault);
		addParentDomains(this.parentEditor);
		...
		await Promise.all(Array.from(targets, async ([target, label]) => {
			try {
				await target.close();
			} catch (error) {
...
*/
var VersionedStorageDomain = class extends require_base$1.StorageDomain {
	/**
...
var GitHistory = class {
	...
	*/
	relativeToRepo(dir, filename) {
		...
		const relDir = (0, path.relative)((0, fs.realpathSync)(root), (0, fs.realpathSync)(dir));
	...
	*/
	exec(cwd, args) {
		return new Promise((resolve, reject) => {
			(0, child_process.execFile)("git", args, {
				cwd,
...
//#endregion
Object.defineProperty(exports, "DOMAIN_KEYS", {
	enumerable: true,
	get: function() {
		return DOMAIN_KEYS;
	}
...
Object.defineProperty(exports, "InMemoryThreadStateStorage", {
	enumerable: true,
	get: function() {
		return InMemoryThreadStateStorage;
	}
});
Object.defineProperty(exports, "MastraCompositeStore", {
	enumerable: true,
	get: function() {
		return MastraCompositeStore;
	}
});
...
Object.defineProperty(exports, "getSourceAgentFilePath", {
	enumerable: true,
	get: function() {
		return getSourceAgentFilePath;
	}
});
Object.defineProperty(exports, "getSourceControlEntityFilePath", {
	enumerable: true,
	get: function() {
		return getSourceControlEntityFilePath;
	}
});
Object.defineProperty(exports, "normalizePerPage", {
	enumerable: true,
	get: function() {
		return normalizePerPage;
	}
});

//# sourceMappingURL=filesystem-versioned-fXZL7zgZ.cjs.map