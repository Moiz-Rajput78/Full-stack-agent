...
const createUsageContract = (toolId) => `# Code Mode
...
function createCodeModeInstructions(config) {
	...
	return `${createUsageContract(config.id ?? "execute_typescript")}\n\n${declarations}`;
}
//#endregion
	...
	return require_tool.createTool({
		...
		execute: async ({ code }, ctx) => {
			...
			if (!sandbox && transport.requiresSandbox !== false) {
				const requestContext = ctx?.requestContext ?? new require_request_context.RequestContext();
				sandbox = await ctx?.workspace?.resolveSandbox({ requestContext });
				if (!sandbox) throw new Error("Code Mode requires a sandbox to run model-authored code, but none was configured. Pass one to createCodeMode({ tools, sandbox }), or run the agent in a workspace that provides a sandbox. To execute on the host (host privileges — only for trusted/local use), pass `sandbox: new LocalSandbox()`.");
			}
...
//# sourceMappingURL=tools-CKkdrGLA.cjs.map