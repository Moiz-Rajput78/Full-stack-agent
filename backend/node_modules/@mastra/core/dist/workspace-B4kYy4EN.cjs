...
const require_interop = require("./interop-vh1X3gt8.cjs");
const require_errors = require("./errors-Bfv4VhVf.cjs");
const require_workspace_skills = require("./workspace-skills-DLF-MW-v.cjs");
...
* {@link interopDefault}.
*/
const pMap = require_interop.interopDefault(p_map.default);
//#endregion
...
function whichSync(binary) {
	try {
		(0, child_process.execFileSync)(process.platform === "win32" ? "where" : "which", [binary], { stdio: "ignore" });
		return true;
	} catch {
		return false;
	}
...
function resolveRequire(root, moduleId) {
	try {
		const req = (0, module$1.createRequire)((0, url.pathToFileURL)((0, path.join)(root, "package.json")));
		return {
			require: req,
	...
	try {
		const req = (0, module$1.createRequire)((0, url.pathToFileURL)((0, path.join)(process.cwd(), "package.json")));
		return {
			require: req,
			resolved: req.resolve(moduleId)
		};
...
function resolveRequireFromPaths(root, moduleId, searchPaths) {
	...
	for (const searchPath of searchPaths ?? []) try {
		const req = (0, module$1.createRequire)((0, url.pathToFileURL)((0, path.join)(searchPath, "package.json")));
		return {
			require: req,
			resolved: req.resolve(moduleId)
		};
...
function resolveNodeBin(root, binary, searchPaths) {
	const local = (0, path.join)(root, "node_modules", ".bin", binary);
	const cwd = (0, path.join)(process.cwd(), "node_modules", ".bin", binary);
	if ((0, fs.existsSync)(local)) return local;
	if ((0, fs.existsSync)(cwd)) return cwd;
	for (const dir of searchPaths ?? []) {
		const p = (0, path.join)(dir, "node_modules", ".bin", binary);
		if ((0, fs.existsSync)(p)) return p;
	}
}
/**
...
function walkUp(startDir, markers) {
	...
	const fsRoot = (0, path.parse)(current).root;
	while (true) {
		for (const marker of markers) if ((0, fs.existsSync)((0, path.join)(current, marker))) return current;
...
*/
async function walkUpAsync(startDir, markers, fs$2) {
	let current = startDir;
	const fsRoot = (0, path.parse)(current).root;
	while (true) {
		for (const marker of markers) if (await fs$2.exists((0, path.join)(current, marker))) return current;
		if (current === fsRoot) break;
		const parent = (0, path.dirname)(current);
		if (parent === current) break;
		current = parent;
	...
	const builtins = {
		typescript: {
			...
			command: (root) => {
				...
				const tscEntry = (0, path.join)((0, path.dirname)(pkg.resolved), binRel);
				if (!(0, fs.existsSync)(tscEntry)) return void 0;
...
var ProcessHandle = class {
	...
	*/
	...
	get writer() {
		if (!this._writer) this._writer = new stream.Writable({
...
var LocalProcessManager = class extends SandboxProcessManager {
	async spawn(command, options = {}) {
		...
		if (options.cwd) if (path.isAbsolute(options.cwd)) cwd = options.cwd;
		else {
			const normalizedWorkingDir = path.resolve(this.sandbox.workingDirectory);
			const normalizedOptionsCwd = path.resolve(options.cwd);
			cwd = normalizedOptionsCwd === normalizedWorkingDir || normalizedOptionsCwd.startsWith(`${normalizedWorkingDir}${path.sep}`) ? normalizedOptionsCwd : path.resolve(this.sandbox.workingDirectory, options.cwd);
		}
...
var MountManager = class {
	...
	*/
	hashConfig(config) {
		...
		return (0, crypto.createHash)("sha256").update(normalized).digest("hex").slice(0, 16);
	}
	sortKeysDeep(obj) {
...
* ```
*/
var MastraSandbox = class extends require_base.MastraBase {
...
function commandExists(command) {
	try {
		(0, child_process.execFileSync)("which", [command], { stdio: "ignore" });
		return true;
	} catch {
		return false;
	}
...
function isSeatbeltAvailable() {
	if (os.default.platform() !== "darwin") return false;
...
function isBwrapAvailable() {
	if (os.default.platform() !== "linux") return false;
...
function canonicalizePath(pathStr) {
	try {
		return (0, fs.realpathSync)(pathStr);
	} catch {
		return pathStr;
	}
}
...
function buildBwrapCommand(command, workspacePath, config) {
	...
	for (const path$5 of DEFAULT_READONLY_BINDS) bwrapArgs.push("--ro-bind-try", path$5, path$5);
	for (const path$6 of config.readOnlyPaths ?? []) bwrapArgs.push("--ro-bind", path$6, path$6);
	...
	for (const path$7 of config.readWritePaths ?? []) bwrapArgs.push("--bind", path$7, path$7);
...
var LocalSandbox = class LocalSandbox extends MastraSandbox {
	...
	*/
	...
	constructor(options = {}) {
		...
		this.workingDirectory = require_workspace_skills.expandTilde(options.workingDirectory ?? path.join(process.cwd(), ".sandbox"));
		...
		this._checkpointsDirectory = options.checkpointsDirectory ? require_workspace_skills.expandTilde(options.checkpointsDirectory) : path.join(path.dirname(this.workingDirectory), ".checkpoints");
	}
	/**
	...
	*/
	async start() {
		...
		await fs_promises.mkdir(this.workingDirectory, { recursive: true });
		...
		if (this.isolation === "seatbelt") {
			...
			if (userProvidedPath) {
				...
				let existingProfile;
				try {
					existingProfile = await fs_promises.readFile(userProvidedPath, "utf-8");
				} catch (err) {
				...
				if (existingProfile !== void 0 && !isGeneratedSeatbeltProfile(existingProfile)) this._customSeatbeltProfile = existingProfile;
					...
					await fs_promises.mkdir(path.dirname(userProvidedPath), { recursive: true });
					await fs_promises.writeFile(userProvidedPath, generatedProfile, "utf-8");
				}
			} else {
				const generatedProfile = generateSeatbeltProfile(this.workingDirectory, this._nativeSandboxConfig);
				...
				await fs_promises.mkdir(this._sandboxFolderPath, { recursive: true });
				this._seatbeltProfilePath = path.join(this._sandboxFolderPath, `seatbelt-${configHash}.sb`);
				await fs_promises.writeFile(this._seatbeltProfilePath, generatedProfile, "utf-8");
			}
		}
	...
	*/
	async _seedFromCheckpoint() {
		...
		if ((await fs_promises.readdir(this.workingDirectory).catch(() => [])).length > 0) return;
		...
		for (const name of candidates) {
			...
			try {
				await fs_promises.cp(checkpointDir, this.workingDirectory, { recursive: true });
			} catch (error) {
				if (error.code !== "ENOENT") throw error;
				await fs_promises.rm(this.workingDirectory, {
					recursive: true,
					force: true
				}).catch(() => {});
				await fs_promises.mkdir(this.workingDirectory, { recursive: true });
	...
	*/
	async _checkpointReadable(checkpointDir) {
		for (let attempt = 0; attempt < 3; attempt++) {
			...
			try {
				if ((await fs_promises.stat(checkpointDir)).isDirectory()) return true;
				return false;
			} catch {}
		}
		return false;
	...
	*/
	...
	async _captureCheckpoint(name) {
		...
		await fs_promises.mkdir(this._checkpointsDirectory, { recursive: true });
		...
		try {
			await fs_promises.cp(this.workingDirectory, tmp, { recursive: true });
			try {
				await fs_promises.rename(target, backup);
				targetMoved = true;
			} catch (error) {
				if (error.code !== "ENOENT") throw error;
			}
			await fs_promises.rename(tmp, target);
		} catch (error) {
			await fs_promises.rm(tmp, {
				recursive: true,
				force: true
			}).catch(() => {});
			if (targetMoved) await fs_promises.rename(backup, target).catch(() => {});
			throw error;
		}
		if (targetMoved) await fs_promises.rm(backup, {
			recursive: true,
			force: true
		}).catch(() => {});
	...
	*/
	async destroy() {
		...
		if (this._seatbeltProfilePath && !this._nativeSandboxConfig.seatbeltProfilePath) try {
			await fs_promises.unlink(this._seatbeltProfilePath);
		} catch {}
		this._seatbeltProfilePath = void 0;
		this._customSeatbeltProfile = void 0;
		if (this._sandboxFolderPath) {
			try {
				await fs_promises.rmdir(this._sandboxFolderPath);
			} catch {}
	...
	*/
	async mount(filesystem, mountPath) {
		...
		try {
			if ((await fs_promises.readdir(hostPath)).length > 0) {
			...
			await fs_promises.rmdir(hostPath);
		} catch (err) {
		...
		try {
			await fs_promises.mkdir(path.dirname(hostPath), { recursive: true });
			await fs_promises.symlink(localConfig.basePath, hostPath);
	...
	*/
	async unmount(mountPath) {
		...
		let isSymlink = false;
		try {
			isSymlink = (await fs_promises.lstat(hostPath)).isSymbolicLink();
		} catch {}
		...
		try {
			await fs_promises.unlink(markerPath);
		} catch {}
		if (isSymlink) try {
			await fs_promises.unlink(hostPath);
	...
	*/
	async writeMarkerFile(mountPath, hostPath) {
		...
		try {
			await fs_promises.mkdir(getMarkerDir(), { recursive: true });
			await fs_promises.writeFile(markerFilePath, markerContent, "utf-8");
		} catch {
	...
	*/
	async checkExistingMount(hostPath, newConfig) {
		try {
			const stats = await fs_promises.lstat(hostPath);
			if (stats.isSymbolicLink() && newConfig.type === "local") {
				const linkTarget = await fs_promises.readlink(hostPath).catch(() => null);
				const resolvedTarget = linkTarget ? path.resolve(path.dirname(hostPath), linkTarget) : null;
				const expectedTarget = path.resolve(newConfig.basePath);
	...
	*/
	async hasMarkerFile(hostPath) {
		...
		const markerPath = path.join(getMarkerDir(), filename);
		try {
			await fs_promises.access(markerPath);
			return true;
		} catch {
			return false;
		}
	...
	*/
	async checkMarkerFile(hostPath, newConfig) {
		...
		const markerPath = path.join(getMarkerDir(), filename);
		try {
			const content = await fs_promises.readFile(markerPath, "utf-8");
	...
	*/
	addMountPathToIsolation(mountPath, hostPath) {
		...
		try {
			isolationPath = (0, fs.realpathSync)(hostPath);
		} catch {}
...
function hashContent(content) {
	if (Buffer.isBuffer(content)) return (0, crypto.createHash)("sha256").update(content).digest("hex");
	return (0, crypto.createHash)("sha256").update(content, "utf-8").digest("hex");
}
/**
...
function parseSkillSnapshotFromFiles(files) {
	...
	const parsed = (0, gray_matter.default)(typeof skillMdFile.content === "string" ? skillMdFile.content : skillMdFile.content.toString("utf-8"));
...
function createSkillTool(skills) {
	return require_tool.createTool({
		...
		inputSchema: zod_v4.z.object({ name: zod_v4.z.string().describe("The name or path of the skill to activate. Use the path when multiple skills share the same name.") }),
...
function createSkillSearchTool(skills) {
	return require_tool.createTool({
		...
		inputSchema: zod_v4.z.object({
			query: zod_v4.z.string().describe("The search query"),
			skillNames: zod_v4.z.array(zod_v4.z.string()).optional().describe("Optional list of skill names to search within"),
			topK: zod_v4.z.number().optional().describe("Maximum number of results to return (default: 5)")
		}),
...
function createSkillReadTool(skills) {
	return require_tool.createTool({
		...
		inputSchema: zod_v4.z.object({
			skillName: zod_v4.z.string().describe("The name or path of the skill. Use the path when multiple skills share the same name."),
			path: zod_v4.z.string().describe("Path to the file relative to the skill root (e.g. \"references/colors.md\", \"scripts/run.sh\")"),
			startLine: zod_v4.z.number().optional().describe("Starting line number (1-indexed). If omitted, starts from the beginning."),
			endLine: zod_v4.z.number().optional().describe("Ending line number (1-indexed, inclusive). If omitted, reads to the end.")
		}),
...
var Workspace = class {
	...
	constructor(config) {
		...
		if (config.mounts && Object.keys(config.mounts).length > 0) {
			if (config.filesystem) throw new require_errors.WorkspaceError("Cannot use both \"filesystem\" and \"mounts\"", "INVALID_CONFIG");
			if (this._sandboxResolver) throw new require_errors.WorkspaceError("Cannot use \"mounts\" with a dynamic sandbox resolver. Mounts are attached to a sandbox instance at construction time. Either pass a static sandbox instance, or have your resolver return a sandbox with its mounts already configured.", "INVALID_CONFIG");
			...
			if (/^class\s/.test(Function.prototype.toString.call(config.filesystem))) throw new require_errors.WorkspaceError("filesystem received a class constructor instead of an instance or resolver function. Pass an instance (e.g., new LocalFilesystem(...)) or a resolver function (({ requestContext }) => fs).", "INVALID_CONFIG");
			this._filesystemResolver = config.filesystem;
		} else this._fs = config.filesystem;
		if (config.browser) {
			if (config.browser.providerType !== "cli") throw new require_errors.WorkspaceError(`Workspace.browser requires a CLI provider (providerType: 'cli'), but got '${config.browser.providerType}'. SDK providers should be used with Agent.browser instead.`, "INVALID_CONFIG", this.id);
			this._browser = config.browser;
		}
		if (config.vectorStore && !config.embedder) throw new require_errors.WorkspaceError("vectorStore requires an embedder", "INVALID_SEARCH_CONFIG");
		if (config.bm25 || config.vectorStore && config.embedder) {
			const buildIndexName = () => {
				...
				if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(indexName)) throw new require_errors.WorkspaceError(`Invalid searchIndexName: "${indexName}". Must start with a letter or underscore, and contain only letters, numbers, or underscores.`, "INVALID_SEARCH_CONFIG", this.id);
				if (indexName.length > 63) throw new require_errors.WorkspaceError(`searchIndexName exceeds 63 characters (got ${indexName.length})`, "INVALID_SEARCH_CONFIG", this.id);
				return indexName;
			};
		...
		if (!this._fs && !this._filesystemResolver && !this._sandbox && !this._sandboxResolver && !this.hasSkillsConfig()) throw new require_errors.WorkspaceError("Workspace requires at least a filesystem, sandbox, or skills", "NO_PROVIDERS");
	}
	generateId() {
	...
	*/
	get skills() {
		...
		if (!this._skills) {
			const source = this._config.skillSource ?? this._fs ?? new require_workspace_skills.LocalSkillSource();
			this._skills = new require_workspace_skills.WorkspaceSkillsImpl({
				source,
	...
	*/
	async index(path$4, content, options) {
		this.assertSearchWritable();
		if (!this._searchEngine) throw new require_errors.SearchNotAvailableError();
		this.lastAccessedAt = /* @__PURE__ */ new Date();
		const doc = {
			id: path$4,
			content,
	...
	*/
	async rebuildSearchIndex(paths) {
		...
		for (const pathOrGlob of paths) try {
			const resolved = await require_workspace_skills.resolvePathPattern(pathOrGlob, readdir);
	...
	*/
	assertSearchWritable() {
		if (this._teardownStarted) throw new require_errors.WorkspaceNotReadyError(this.id, this._status);
	}
	/**
	...
	*/
	async getInfo(options) {
		...
		const filesystem = this.filesystem ?? (this._filesystemResolver && shouldResolveDynamicProviders ? await this.resolveFilesystem({ requestContext: options?.requestContext ?? new require_request_context.RequestContext() }) : void 0);
		if (filesystem) {
	...
	*/
	async getInstructionsAsync(opts) {
		const requestContext = opts?.requestContext ?? new require_request_context.RequestContext();
...
function requireWorkspace(context) {
	if (!context?.workspace) throw new require_errors.WorkspaceNotAvailableError();
...
function requireFilesystem(context) {
	...
	if (!workspace.filesystem) throw new require_errors.FilesystemNotAvailableError();
	return {
		workspace,
...
function requireSandbox(context) {
	const workspace = requireWorkspace(context);
	if (!workspace.sandbox) throw new require_errors.SandboxNotAvailableError();
	return {
		workspace,
...
async function getEditDiagnosticsText(workspace, filePath, content) {
	try {
		...
		const absolutePath = workspace.filesystem?.resolveAbsolutePath?.(filePath) ?? (isWindowsRoot ? path.default.win32.resolve(lspManager.root, filePath.replace(/^\/+/, "")) : path.default.posix.resolve(lspManager.root, filePath.replace(/^\/+/, "")));
...
function isAstGrepAvailable() {
	...
	try {
		(0, module$1.createRequire)(require("url").pathToFileURL(__filename).href).resolve("@ast-grep/napi");
		return true;
	} catch {
		return false;
	}
...
const astEditTool = require_tool.createTool({
...
Pattern replace (for everything else):
  { pattern: "console.log($ARG)", replacement: "logger.debug($ARG)" }`,
	...
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The path to the file to edit"),
		pattern: zod_v4.z.string().optional().describe("AST pattern to search for (supports $VARIABLE placeholders, e.g., \"console.log($ARG)\")"),
		replacement: zod_v4.z.string().optional().describe("Replacement pattern (can use captured $VARIABLES, e.g., \"logger.debug($ARG)\")"),
		transform: zod_v4.z.enum([
			"add-import",
			"remove-import",
			"rename"
		]).optional().describe("Structured transformation to apply"),
		targetName: zod_v4.z.string().optional().describe("Required for remove-import and rename transforms. The current name to target."),
		newName: zod_v4.z.string().optional().describe("Required for rename transform. The new name to replace targetName with."),
		importSpec: zod_v4.z.object({
			module: zod_v4.z.string().describe("Module to import from"),
			names: zod_v4.z.array(zod_v4.z.string()).min(1).describe("Names to import. For default imports, put the default name first."),
			isDefault: zod_v4.z.boolean().optional().describe("Whether the first name is a default import")
	...
	execute: async ({ path, pattern, replacement, transform, targetName, newName, importSpec }, context) => {
		...
		try {
			if (filesystem.readOnly) throw new require_errors.WorkspaceReadOnlyError("ast_edit");
			...
			try {
				...
				if (error instanceof require_errors.FileNotFoundError) {
					span.end({ success: false });
...
const deleteFileTool = require_tool.createTool({
	id: WORKSPACE_TOOLS.FILESYSTEM.DELETE,
	description: "Delete a file or directory from the workspace filesystem",
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The path to the file or directory to delete"),
		recursive: zod_v4.z.boolean().optional().default(false).describe("If true, delete directories and their contents recursively. Required for non-empty directories.")
	}),
	execute: async ({ path, recursive }, context) => {
		...
		try {
			if (filesystem.readOnly) throw new require_errors.WorkspaceReadOnlyError("delete");
...
const editFileTool = require_tool.createTool({
...
- Use replace_all only when intentionally replacing all occurrences.`,
	...
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The path to the file to edit"),
		old_string: zod_v4.z.string().describe("The exact text to find and replace. Must be unique in the file."),
		new_string: zod_v4.z.string().describe("The text to replace old_string with"),
		replace_all: zod_v4.z.boolean().optional().default(false).describe("If true, replace all occurrences. If false (default), old_string must be unique.")
	}),
	execute: async ({ path, old_string, new_string, replace_all }, context) => {
		...
		try {
			if (filesystem.readOnly) throw new require_errors.WorkspaceReadOnlyError("edit_file");
...
async function applyTokenLimit(output, limit = DEFAULT_MAX_OUTPUT_TOKENS, from = "start") {
	if (!output) return output;
	const totalTokens = (0, tokenx.estimateTokenCount)(output);
	if (totalTokens <= limit) return output;
	const kept = from === "start" ? (0, tokenx.sliceByTokens)(output, -limit) : (0, tokenx.sliceByTokens)(output, 0, limit);
...
async function applyTokenLimitSandwich(output, limit = DEFAULT_MAX_OUTPUT_TOKENS, headRatio = .1) {
	...
	const head = headBudget > 0 ? (0, tokenx.sliceByTokens)(output, 0, headBudget) : "";
	const tail = tailBudget > 0 ? (0, tokenx.sliceByTokens)(output, -tailBudget) : "";
	return [
		head,
...
const executeCommandInputSchema = zod_v4.z.object({
	...
	tail: zod_v4.z.preprocess(coerceNumericString, zod_v4.z.number()).nullish().describe(`For foreground commands: limit output to the last N lines, similar to tail -n. Defaults to 200. Use 0 for no limit.`)
});
/** Schema with background param included. */
const executeCommandWithBackgroundSchema = executeCommandInputSchema.extend({ background: zod_v4.z.boolean().optional().describe("Run the command in the background. Returns a PID immediately instead of waiting for completion. Use get_process_output to check on it later.") });
/**
...
async function executeCommand(input, context) {
	...
	if (background) {
		if (!sandbox.processes) {
			const err = new require_errors.SandboxFeatureNotSupportedError("processes");
			span.error(err);
			throw err;
		}
	...
	if (!sandbox.executeCommand) {
		const err = new require_errors.SandboxFeatureNotSupportedError("executeCommand");
		span.error(err);
		throw err;
	}
...
const executeCommandTool = require_tool.createTool({
...
const executeCommandWithBackgroundTool = require_tool.createTool({
	id: WORKSPACE_TOOLS.SANDBOX.EXECUTE_COMMAND,
	description: `${baseDescription}
...
const fileStatTool = require_tool.createTool({
	...
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({ path: zod_v4.z.string().describe("The path to check") }),
...
const getProcessOutputTool = require_tool.createTool({
...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
	...
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({
		pid: zod_v4.z.string().describe("The process ID returned when the background command was started"),
		tail: zod_v4.z.preprocess(coerceNumericString, zod_v4.z.number()).optional().describe(`Number of lines to return, similar to tail -n. Positive or negative returns last N lines from end. Defaults to 200. Use 0 for no limit.`),
		wait: zod_v4.z.boolean().optional().describe("If true, block until the process exits and return the final output. Useful for short-lived background commands where you want to wait for the result.")
	}),
...
async function loadGitignore(filesystem) {
	...
	const ig = (0, ignore.default)().add(content);
	return (relativePath) => {
...
const grepTool = require_tool.createTool({
...
- With context: { pattern: "function", contextLines: 2 }`,
	...
	execute: async ({ pattern, path: inputPath = ".", contextLines = 0, maxCount, caseSensitive = true, includeHidden = false }, context) => {
		...
		try {
			...
			let searchPath;
			let globMatcher;
			if (require_workspace_skills.isGlobPattern(inputPath)) {
				searchPath = require_workspace_skills.extractGlobBase(inputPath);
				globMatcher = require_workspace_skills.createGlobMatcher(inputPath, { dot: includeHidden });
			...
			if (normalizedSearch === ".git" || normalizedSearch.endsWith("/.git")) filePaths = [];
				...
				if ((await filesystem.stat(searchPath)).type === "file") filePaths = require_workspace_skills.isTextFile(searchPath) ? [searchPath] : [];
				else {
					const collectFiles = async (dir) => {
						...
						for (const entry of entries) {
							...
							if (entry.type === "file") {
								if (!require_workspace_skills.isTextFile(entry.name)) continue;
...
const indexContentTool = require_tool.createTool({
	id: WORKSPACE_TOOLS.SEARCH.INDEX,
	description: "Index content for search. The path becomes the document ID in search results.",
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The document ID/path for search results"),
		content: zod_v4.z.string().describe("The text content to index"),
		metadata: zod_v4.z.record(zod_v4.z.string(), zod_v4.z.unknown()).optional().describe("Optional metadata to store with the document")
	}),
...
const killProcessTool = require_tool.createTool({
...
Use this to stop a long-running background process that was started with execute_command (background: true). Returns the last ${KILL_TAIL_LINES} lines of output.`,
	toModelOutput: sandboxToModelOutput,
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({ pid: zod_v4.z.string().describe("The process ID of the background process to kill") }),
...
async function formatAsTree(fs, path, options) {
	...
	let globMatcher;
	if (pattern) globMatcher = require_workspace_skills.createGlobMatcher(Array.isArray(pattern) ? pattern : [pattern], { dot: showHidden });
	const lines = ["."];
	const paths = [];
	let dirCount = 0;
	let fileCount = 0;
...
const listFilesTool = require_tool.createTool({
...
To list ALL files, omit the pattern parameter — do NOT pass pattern: "*".`,
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().default(".").describe("Directory path to list"),
		maxDepth: zod_v4.z.number().optional().default(2).describe("Maximum depth to descend (default: 2). Similar to tree -L flag."),
		showHidden: zod_v4.z.boolean().optional().default(false).describe("Show hidden files starting with \".\" (default: false). Similar to tree -a flag."),
		dirsOnly: zod_v4.z.boolean().optional().default(false).describe("List directories only, no files (default: false). Similar to tree -d flag."),
		exclude: zod_v4.z.string().optional().describe("Pattern to exclude (e.g., \"node_modules\"). Similar to tree -I flag."),
		extension: zod_v4.z.string().optional().describe("Filter by file extension (e.g., \".ts\"). Similar to tree -P flag."),
		...
		respectGitignore: zod_v4.z.boolean().optional().default(true).describe("Respect .gitignore in the listed directory (default: true).")
	}),
...
async function getLinePreview(filePath, lineNumber) {
	try {
		return (await fs_promises.default.readFile(filePath, "utf-8")).split("\n")[lineNumber - 1]?.trim() ?? null;
	} catch {
		return null;
	}
}
function getAbsolutePath(workspacePath, lspRoot, resolveAbsolutePath) {
	...
	if (path.default.isAbsolute(workspacePath)) return workspacePath;
	return path.default.resolve(lspRoot, workspacePath);
}
function locationUriToPath(uri) {
	if (!uri.startsWith("file://")) return null;
	try {
		return (0, url.fileURLToPath)(uri);
	} catch {
		return null;
	}
}
...
const lspInspectTool = require_tool.createTool({
	...
	outputSchema: zod_v4.z.object({
		hover: zod_v4.z.object({
			value: zod_v4.z.string(),
			kind: zod_v4.z.string()
		}).optional(),
		diagnostics: zod_v4.z.array(zod_v4.z.object({
			severity: zod_v4.z.string(),
			message: zod_v4.z.string(),
			source: zod_v4.z.string().nullable()
		})).optional(),
		definition: zod_v4.z.array(zod_v4.z.object({
			location: zod_v4.z.string(),
			preview: zod_v4.z.string().nullable()
		})).optional(),
		implementation: zod_v4.z.array(zod_v4.z.string()).optional(),
		error: zod_v4.z.string().optional()
	}),
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("Absolute path to the file"),
		line: zod_v4.z.number().int().positive().describe("Line number (1-indexed)"),
		match: zod_v4.z.string().describe("Line content with <<< marking the cursor position. Exactly one <<< marker is required. Example: \"const foo = <<<bar()\" means cursor is at bar")
	}),
	execute: async ({ path: filePath, line, match }, context) => {
		...
		let fileContent = "";
		try {
			fileContent = await fs_promises.default.readFile(absolutePath, "utf-8");
		} catch {
			fileContent = "";
		}
		let queryResult;
...
const mkdirTool = require_tool.createTool({
	id: WORKSPACE_TOOLS.FILESYSTEM.MKDIR,
	description: "Create a directory in the workspace filesystem",
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The path of the directory to create"),
		recursive: zod_v4.z.boolean().optional().default(true).describe("Whether to create parent directories if they do not exist")
	}),
	execute: async ({ path, recursive }, context) => {
		const { workspace, filesystem } = requireFilesystem(context);
		...
		try {
			if (filesystem.readOnly) throw new require_errors.WorkspaceReadOnlyError("mkdir");
...
const readFileTool = require_tool.createTool({
	...
	outputSchema: zod_v4.z.union([zod_v4.z.string(), zod_v4.z.object({
		__workspaceMedia: zod_v4.z.literal(true),
		text: zod_v4.z.string(),
		mediaType: zod_v4.z.string(),
		data: zod_v4.z.string()
	})]),
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The path to the file to read (e.g., \"data/config.json\")"),
		offset: zod_v4.z.number().int().min(1).optional().describe("Line number to start reading from (1-indexed). Only used when reading text files; ignored for media and other binary files. Defaults to line 1 if omitted."),
		limit: zod_v4.z.number().int().min(1).optional().describe("Maximum number of lines to read. Only used when reading text files; ignored for media and other binary files. Defaults to the end of the file if omitted."),
		showLineNumbers: zod_v4.z.boolean().optional().default(true).describe("Prefix each line with its line number. Only used when reading text files; ignored for media and other binary files. Defaults to true if omitted."),
		encoding: zod_v4.z.enum([
			"utf-8",
			"utf8",
			"base64",
			"hex",
...
const searchInputSchema = zod_v4.z.object({
	query: zod_v4.z.string().describe("The search query string"),
	topK: zod_v4.z.number().optional().default(5).describe("Maximum number of results to return"),
	mode: zod_v4.z.enum([
		"bm25",
		"vector",
		"hybrid"
	]).optional().describe("Search mode: bm25 for keyword search, vector for semantic search, hybrid for both combined"),
	minScore: zod_v4.z.number().optional().describe("Minimum score threshold (0-1 for normalized scores)")
});
const searchTool = require_tool.createTool({
...
const writeFileTool = require_tool.createTool({
	...
	outputSchema: zod_v4.z.string(),
	inputSchema: zod_v4.z.object({
		path: zod_v4.z.string().describe("The path where to write the file (e.g., \"data/output.txt\")"),
		content: zod_v4.z.string().describe("The content to write to the file"),
		overwrite: zod_v4.z.boolean().optional().default(true).describe("Whether to overwrite the file if it already exists")
	}),
	execute: async ({ path, content, overwrite }, context) => {
		...
		try {
			if (filesystem.readOnly) throw new require_errors.WorkspaceReadOnlyError("write_file");
...
async function resolveEffectiveWorkspace(workspace, context, targets) {
	...
	const requestContext = context?.requestContext ?? new require_request_context.RequestContext();
	const overrides = {};
	if (needsFilesystem) {
				...
				try {
					...
					if (config.requireReadBeforeWrite !== void 0) {
						if (await resolveDynamicValue(config.requireReadBeforeWrite, {
							...
							if (check.needsReRead) throw new require_errors.FileReadRequiredError(input.path, check.reason);
						}
					}
				} catch (error) {
					if (!(error instanceof require_errors.FileNotFoundError)) throw error;
				}
			}
...
async function createWorkspaceTools(workspace, configContext) {
	...
	if (workspace.canBM25 || workspace.canVector) {
		...
		const dynamicSearchTool = {
			...searchTool,
			inputSchema: searchInputSchema.extend({ mode: zod_v4.z.enum(availableModes).optional().describe(`Search mode: ${availableModes.join(", ")}`) })
		};
...
//#endregion
Object.defineProperty(exports, "BM25Index", {
	enumerable: true,
	get: function() {
		return BM25Index;
	}
...
Object.defineProperty(exports, "CompositeVersionedSkillSource", {
	enumerable: true,
	get: function() {
		return CompositeVersionedSkillSource;
	}
});
...
Object.defineProperty(exports, "UnsupportedStdinCloseError", {
	enumerable: true,
	get: function() {
		return UnsupportedStdinCloseError;
	}
});
Object.defineProperty(exports, "VersionedSkillSource", {
	enumerable: true,
	get: function() {
		return VersionedSkillSource;
	}
});
...
Object.defineProperty(exports, "indexContentTool", {
	enumerable: true,
	get: function() {
		return indexContentTool;
	}
});
Object.defineProperty(exports, "isIsolationAvailable", {
	enumerable: true,
	get: function() {
		return isIsolationAvailable;
	}
});
...
Object.defineProperty(exports, "pMap", {
	enumerable: true,
	get: function() {
		return pMap;
	}
...
Object.defineProperty(exports, "supportsNetworking", {
	enumerable: true,
	get: function() {
		return supportsNetworking;
	}
});
...
//# sourceMappingURL=workspace-B4kYy4EN.cjs.map