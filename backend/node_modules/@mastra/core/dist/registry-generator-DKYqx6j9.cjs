...
async function atomicWriteFile(filePath, content, encoding = "utf-8") {
	...
	try {
		await fs_promises.default.writeFile(tempPath, content, encoding);
		for (let attempt = 0;; attempt += 1) try {
			await fs_promises.default.rename(tempPath, filePath);
			break;
		} catch (error) {
			const delayMs = WINDOWS_RENAME_RETRY_DELAYS_MS[attempt];
			if (!(process.platform === "win32" && error instanceof Error && "code" in error && (error.code === "EPERM" || error.code === "EBUSY")) || delayMs === void 0) throw error;
			await new Promise((resolve) => setTimeout(resolve, delayMs));
		}
	} catch (error) {
		try {
			await fs_promises.default.unlink(tempPath);
		} catch {}
		throw error;
	}
}
...
async function fetchProvidersFromGateways(gateways) {
	...
	for (const gateway of gateways) if (require_gateway_helpers.shouldEnableGateway(gateway)) enabledGateways.push(gateway);
	const allProviders = {};
	const allModels = {};
	...
	for (const gateway of enabledGateways) {
		...
		if (!providers) {
			failedGateways.push(require_gateway_helpers.getGatewayId(gateway));
			continue;
		}
...
async function writeRegistryFiles(jsonPath, typesPath, providers, models, attachmentCapabilities, temperatureCapabilities, structuredOutputCapabilities) {
	const jsonDir = path.default.dirname(jsonPath);
	const typesDir = path.default.dirname(typesPath);
	await fs_promises.default.mkdir(jsonDir, { recursive: true });
	await fs_promises.default.mkdir(typesDir, { recursive: true });
	...
	if (attachmentCapabilities && Object.keys(attachmentCapabilities).length > 0 || temperatureCapabilities && Object.keys(temperatureCapabilities).length > 0 || structuredOutputCapabilities && Object.keys(structuredOutputCapabilities).length > 0) {
		const capDir = path.default.join(jsonDir, "capabilities");
		await fs_promises.default.rm(capDir, {
			recursive: true,
			force: true
		});
		await fs_promises.default.mkdir(capDir, { recursive: true });
		...
		for (const provider of allProviderIds) {
			...
			await atomicWriteFile(path.default.join(capDir, require_gateway_helpers.getCapabilityFileName(provider)), JSON.stringify(capData, null, 2), "utf-8");
		}
	}
}
//#endregion
...
//# sourceMappingURL=registry-generator-DKYqx6j9.cjs.map