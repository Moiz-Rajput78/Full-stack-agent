...
- [Filesystem](references/docs-sandbox-filesystem.md) - Give agents access to local or remote files and persistent sandbox storage.
- [LSP inspection](references/docs-sandbox-lsp.md) - Configure Language Server Protocol support for semantic inspection of project files.
- [Sandboxes](references/docs-sandbox-overview.md) - Give agents an isolated environment for working with files and running commands.
- [Search and indexing](references/docs-sandbox-search.md) - Index filesystem or application content for BM25 keyword search, vector semantic search, or hybrid mode.
...
- [Reference: dataset.createExperiment()](references/reference-datasets-createExperiment.md) - Documentation for the `dataset.createExperiment()` method, which creates an experiment whose loop is owned by the caller.
...
- [Reference: dataset.finalizeExperiment()](references/reference-datasets-finalizeExperiment.md) - Documentation for the `dataset.finalizeExperiment()` method, which computes final counts for a caller-driven experiment and marks it completed.
...
- [Reference: dataset.runExperimentItem()](references/reference-datasets-runExperimentItem.md) - Documentation for the `dataset.runExperimentItem()` method, which executes one experiment item server-side with upsert semantics.
...
- [Reference: dataset.submitExperimentResult()](references/reference-datasets-submitExperimentResult.md) - Documentation for the `dataset.submitExperimentResult()` method, which ingests one externally computed item result with upsert semantics.
